...
open class HMBaseAudioVC: NSObject, AVAudioPlayerDelegate {
    ...
    private func startProgressTimer() {
        progressTimer?.invalidate()
        progressTimer = nil
        progressTimer = Timer.scheduledTimer(timeInterval: 0.1, target: self, selector: #selector(HMBaseAudioVC.didFireProgressTimer(_:)), userInfo: nil, repeats: true)
    }
